...
class PluginManager:
    ...
    def generation_system_messages(self, language=None, context=None):
        """Transient instructions from enabled plugins, never chat history."""
        messages = []
        for plugin in self.iter_all_plugins():
            hook = getattr(plugin, 'generation_prompt', None)
            if callable(hook):
                message = hook(language=language, context=context)
                if isinstance(message, str) and message.strip():
                    messages.append(message.strip())
        return messages

    def generation_output_guards(self, context=None):
        """Select once, before generation; guards run before TTS and memory saves."""
        guards = []
        for plugin in self.iter_all_plugins():
            enabled = getattr(plugin, 'generation_guard_enabled', None)
            hook = getattr(plugin, 'guard_generation_output', None)
            if callable(enabled) and callable(hook) and enabled(context):
                guards.append(hook)
        return guards
    ...
    def handle_before_chat(self, user_input: str, context=None):
        ...
        for plugin in self.plugins_chat:
            ...
            if callable(before_fn):
                from shared.core.chat_turn import DEFERRED_CHAT_PLUGINS
                turn = context.get('gui_chat_turn')
                if turn and not turn.committed and getattr(plugin, 'plugin_id', '') in DEFERRED_CHAT_PLUGINS:
                    turn.defer(lambda fn=before_fn, text=current: fn(text, context))
                    continue
                try:
    ...
    def handle_after_response(self, reply: str, context=None):
        context = context or {}
        from shared.core.maat_assessment import context_messages, correct_average
        messages = context_messages(context)
        current = correct_average(reply, messages)
        ...
        return correct_average(current, messages)
