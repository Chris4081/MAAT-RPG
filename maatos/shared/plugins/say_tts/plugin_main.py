...
from shared.core.thinking_mode import show_thinking_enabled
...
class Plugin:
    ...
    def _tts_worker(self):
        ...
        while not self.stop_event.is_set():
            ...
            with self._speech_lock:
                if generation != self._speech_generation or self.stop_event.is_set():
                    continue
                try:
    ...
    def begin_response_speech(self):
        """Called on the first new token, never merely on the user's input."""
        self._reset_speech(cancelled=False)

    def cancel_speech(self):
        """Stop this answer, including queued sentences and late stream hooks."""
        self._reset_speech(cancelled=True)

    def _reset_speech(self, *, cancelled):
        with self._speech_lock:
            self._speech_cancelled = cancelled
    ...
    def _speak(self, text: str):
        ...
        if self.enabled and prepared:
            self._sync_voice_with_language()
            with self._speech_lock:
                if not self._speech_cancelled:
                    self.tts_queue.put((self._speech_generation, prepared))
    ...
    def _on_token(self, token):
        if token and self._pending_response:
            self.begin_response_speech()
        if not self.enabled or self._speech_cancelled:
            return
    ...
    def after_final_response(self, reply, context=None):
        self.enabled = self._tts_enabled_setting() and bool(self.tts_backend)
        if self.enabled and isinstance(reply, str) and reply.strip() and reply != self._last_stream_text:
            self._speak(reply)
        return reply
