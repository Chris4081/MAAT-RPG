...
from shared.core.ai_plugin_settings import settings_for, clear_emotion
...
class EmotionEngineV2:
    def __init__(self):
        ...
        english = {
            'joy': ['happy', 'delighted', 'joy', 'excited'],
            'gratitude': ['thanks', 'thank you', 'grateful', 'appreciate'],
            'affection': ['love', 'fond', 'hug'], 'calm': ['calm', 'relaxed', 'peaceful'],
            'inspiration': ['inspired', 'inspiring', 'brilliant', 'idea'],
            'connection': ['connected', 'together', 'close to'],
            'presence': ['present', 'right now'], 'sadness': ['sad', 'unhappy', 'miss you', 'grieving'],
            'overload': ['overwhelmed', 'exhausted', 'stressed'],
            'fear': ['afraid', 'worried', 'anxious', 'fear', 'panic'],
            'anger': ['angry', 'frustrated', 'furious'],
        }
        for emotion, words in english.items():
            self.EMO_MAP[emotion] = list(dict.fromkeys(self.EMO_MAP[emotion] + words))
    ...
    def detect_raw(self, text: str):
        ...
        return dominant[0], min(1.0, dominant[1] / 10)
    ...
    def safe(self, e_raw, E_value, intensity=1.0, language='de'):
        if language == 'en':
            if e_raw == 'neutral':
                return 'No clear emotional cue detected in the text.'
            return f'The wording may suggest {self.transform(e_raw, language)}. This is only a keyword-based estimate.'
        if e_raw != 'neutral':
            return f'Die Wortwahl könnte {self.transform(e_raw)} wirken. Das ist nur eine Einschätzung anhand von Stichwörtern.'
        return 'Keine eindeutigen emotionalen Hinweise im Text erkannt.'

    def transform(self, e_raw, language='de'):
        if language == 'en':
            return {'joy':'joy', 'gratitude':'gratitude', 'affection':'affection', 'calm':'calm',
                    'inspiration':'inspiration', 'connection':'connection', 'presence':'presence',
                    'sadness':'sadness', 'overload':'overwhelm', 'fear':'anxiety', 'anger':'frustration'}.get(e_raw,e_raw)
        return {'joy':'freudig', 'gratitude':'dankbar', 'affection':'liebevoll', 'calm':'gelassen',
                'inspiration':'inspiriert', 'connection':'verbunden', 'presence':'präsent',
                'sadness':'traurig', 'overload':'überfordert', 'fear':'ängstlich', 'anger':'frustriert'}.get(e_raw,e_raw)
...
class Plugin:
    ...
    def command(self, full_cmd: str, context=None):
        ...
        if cmd == "/emotion":
            if not settings_for().get('emotion_enabled', True):
                self.last_result = None
                return True, self._t('Emotionserkennung ist deaktiviert.', 'Emotion detection is disabled.')
            ...
            return True, (
                ...
                + f"{self._t('Roh-Emotion', 'Raw emotion')}: {self.engine.transform(r['emotion_raw'], self._lang())}  ({self._t('Intensitaet', 'Intensity')}: {r['intensity']:.2f})\n"
                f"E_value: {r['E_value']:+.3f}\n"
                f"A_from_emotion: {r['A_from_emotion']:.3f}\n"
                f"ΔD_from_emotion: {r['deltaD_from_emotion']:.3f}\n\n"
                f"{self._t('Interpretation', 'Interpretation')}: {self.engine.safe(r['emotion_raw'], r['E_value'], r['intensity'], self._lang())}"
            )
    ...
    def before_chat(self, user_input: str, context=None):
        ...
        """
        if not settings_for(context).get('emotion_enabled', True):
        ...
        safe_text = self.engine.safe(e_raw, E_value, intensity=intensity, language=self._lang())
