...
def load_backend(
    ...
    if load_options is not None:
        llama_kwargs['load_options'] = load_options
    if not allow_cpu_fallback:
        llama_kwargs['allow_cpu_fallback'] = False
    ...
    if backend == "llama_intel":
        from .intel_gguf_backend import load as intel_load
        return intel_load(model_path, **llama_kwargs)
...
def stream_chat(llm, messages, perf=None):
    ...
    if backend == "llama_intel":
        from .intel_gguf_backend import stream_chat as intel_stream
        return intel_stream(llm, messages, perf)
