...
def music_enabled(default: bool = False) -> bool:
    try:
...
def sound_enabled() -> bool:
    """Short project-made cues have their own switch, separate from music."""
    try:
        with open(state_file("settings_state.json"), "r", encoding="utf-8") as f:
            return bool(json.load(f).get("gui_sound_enabled", True))
