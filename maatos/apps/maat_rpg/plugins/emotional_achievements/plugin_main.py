...
from shared.core.achievement_i18n import EMOTIONS, matches_phrase
...
def grant_rpg_xp(plugin_dir: str, amount: int):
    ...
    try:
        ...
        state_path = state_file('battle_state.json')
        ...
        if lvl > old_lvl:
            print(
                Fore.YELLOW
                + (f"\n🌟 LEVEL UP through emotion! Level {old_lvl} → {lvl}  (+{amount} XP)" if get_language(("de","en"))=='en'
        ...
        print(
            Fore.RED
            + (f"[Emotional achievements → RPG XP ERROR]: {e}" if get_language(("de","en"))=='en'
               else f"[Emotional-Achievements → RPG-XP FEHLER]: {e}")
            + Style.RESET_ALL
        )
...
def print_rpg_xp_bar(plugin_dir: str):
    ...
    try:
        ...
        if gui_bridge.enabled():
            gui_bridge.emit('level_progress', level=lvl, xp=xp, next_xp=need, gain=None)
            return
...
class Plugin:
    ...
    def __init__(self):
        ...
        self.state_path = state_file('achievements.json')
    ...
    def command(self, cmd, context=None):
        if cmd.strip().lower() == "/ach":
            unlocked = [(EMOTIONS[k][0] if self._lang()=='en' else ACHIEVEMENTS[k]['name'])
                        for k,v in self.state['achievements'].items() if v and k in ACHIEVEMENTS]
    ...
    def _check_achievements(self, text):
        ...
        for key, data in ACHIEVEMENTS.items():
            ...
            if english:
                if matches_phrase(text,EMOTIONS[key][2]):results.append(key)
                continue
            # Existing German full-text heuristic.
            for w in data["check"]:
                if w in lt:
                    results.append(key)
                    break
    ...
    def before_chat(self, user_input, context=None):
        ...
        for key in unlocked_now:
            ...
            total_xp += ach["xp"]
            name,quote=(EMOTIONS[key][0],EMOTIONS[key][1]) if english else (ach['name'],ach.get('msg',ach['name']))
