...
from shared.core.achievement_i18n import WORDS, matches_phrase
...
ACHIEVEMENT_TITLE_EN = {key:value[0] for key,value in WORDS.items()}
...
class Plugin:
    ...
    def _combat_achievement_title(self, key: str) -> str:
        if self._lang() == "en":
            return COMBAT_ACHIEVEMENTS_EN.get(key, COMBAT_ACHIEVEMENTS.get(key, key))
        return COMBAT_ACHIEVEMENTS.get(key, key)
    ...
    def _render_achievements(self):
        ...
        if combat:
            ...
            lines.append("")
            for key in combat:
                lines.append(f"🏆 {self._combat_achievement_title(key)}")
    ...
    def before_chat(self, user_input, context=None):
        ...
        for word, (title, reward) in TRIGGERS.items():
            matches=matches_phrase(user_input, WORDS[word][1]) if english else word in txt
            if matches:
                # nur 1× pro Wort
