...
import sys
import json
from apps.maat_rpg.session_shared import PROFILE_SLOT_COUNT
import subprocess
import select
import time
import threading
...
TEXT = {
    "de": {
        ...
        "opt_thinking": "[4] Thinking-Modus: {value}",
        "opt_rpg_context": "[5] Story- & Kampfkontext: {value} (neue Modelle)",
        "opt_hallu": "[6] Hallu-Modus: {value}",
        "opt_reset": "[7] Zaehler zuruecksetzen (Story + Battle)",
        "opt_full_reset": "[8] Alles zuruecksetzen (Story + Battle + Self-Evo)",
        "opt_memory": "[9] Alle Erinnerungen loeschen (Application Support/MAAT-RPG/data)",
        "opt_language": "[10] Sprache wechseln",
        "opt_profile_switch": "[11] Profil wechseln: {value}",
        "opt_profile_delete": "[12] Profil loeschen",
        "opt_back": "[13] Zurueck",
        "speed_slow": "Langsam",
        "speed_fast": "Schnell",
        "music_on": "An",
        ...
        "profile_slot": "Profil",
        "profile_change_prompt": "Welches Profil soll aktiv werden? [1-10]: ",
        ...
        "profile_delete_prompt": "Welches Profil soll geloescht werden? [2-10]: ",
    ...
    "en": {
        ...
        "opt_voice": "[3] Voice / TTS: {value}",
        "opt_thinking": "[4] Thinking mode: {value}",
        "opt_rpg_context": "[5] Story & battle context: {value} (new models)",
        "opt_hallu": "[6] Hallu mode: {value}",
        "opt_reset": "[7] Reset counters (Story + Battle)",
        "opt_full_reset": "[8] Reset everything (Story + Battle + Self-Evo)",
        "opt_memory": "[9] Delete all memories (Application Support/MAAT-RPG/data)",
        "opt_language": "[10] Change language",
        "opt_profile_switch": "[11] Change profile: {value}",
        "opt_profile_delete": "[12] Delete profile",
        "opt_back": "[13] Back",
        "speed_slow": "Slow",
        "speed_fast": "Fast",
        "music_on": "On",
        ...
        "profile_slot": "Profile",
        "profile_change_prompt": "Which profile should become active? [1-10]: ",
        ...
        "profile_delete_prompt": "Which profile should be deleted? [2-10]: ",
...
def _setting_music_enabled(settings: dict) -> bool:
    return bool(settings.get("music_enabled", False))


def _setting_thinking_enabled(settings: dict) -> bool:
    return bool(settings.get("thinking_enabled", False))
...
def _setting_rpg_context_enabled(settings: dict) -> bool:
    return bool(settings.get("rpg_context_enabled", False))
...
def _settings_labels(language: str, settings: dict) -> tuple[str, str, str, str, str, str]:
    ...
    thinking = t["music_on"] if _setting_thinking_enabled(settings) else t["music_off"]
    rpg_context = t["music_on"] if _setting_rpg_context_enabled(settings) else t["music_off"]
    hallu = t["music_on"] if _setting_hallu_mode(settings) else t["music_off"]
    return speed, music, voice, thinking, rpg_context, hallu
# ==========================
# 🎵 Menü-Musik (optional)
# ==========================
class MenuMusic:
...
def _active_profile_slot() -> int:
    try:
        ...
        return slot if 1 <= slot <= PROFILE_SLOT_COUNT else 1
    except Exception:
        return 1
...
def _profile_slot_label(slot: int, language: str) -> str:
    from apps.maat_rpg.session_shared import profile_slot_label
    return profile_slot_label(slot, language)
...
def _menu_context() -> dict:
    story = _load_state(os.path.basename(story_state_file()))
...
def _change_profile_from_settings(menu_music: MenuMusic, language: str):
    ...
    if choice not in {str(slot) for slot in range(1, PROFILE_SLOT_COUNT + 1)}:
        print(Fore.RED + t["invalid"] + Style.RESET_ALL)
        time.sleep(1)
        return
...
def _delete_profile_from_settings(language: str):
    ...
    if choice not in {str(slot) for slot in range(2, PROFILE_SLOT_COUNT + 1)}:
        print(Fore.RED + t["invalid"] + Style.RESET_ALL)
        time.sleep(1)
        return
...
def options_menu(plugin_dir: str, menu_music: MenuMusic):
    while True:
        ...
        language = settings.get("language", "de")
        t = TEXT.get(language, TEXT["de"])
        speed_label, music_label, voice_label, thinking_label, rpg_context_label, hallu_label = _settings_labels(language, settings)
        active_profile_label = _active_profile_label(language)
        clear_screen()
        ...
        print("  " + t["opt_rpg_context"].format(value=rpg_context_label))
        ...
        if choice == "1":
            ...
            enabled = _setting_thinking_enabled(settings)
            settings["thinking_enabled"] = not enabled
            if not settings["thinking_enabled"]:
                settings["show_thinking"] = False
            _save_settings(settings)
        elif choice == "5":
            enabled = _setting_rpg_context_enabled(settings)
            settings["rpg_context_enabled"] = not enabled
            _save_settings(settings)
        elif choice == "6":
...
class Plugin:
    ...
    def _run_title_demo(self, context=None) -> bool:
        ...
        demo_context = {
            "combat_source": "demo",
            "arena_difficulty": ("easy", "normal", "hard")[cycle],
            "guide_mode": True,
