...
from shared.core.shop_catalog import item_price
from shared.core import talents as class_talents
import os
import json
import random
import time
...
from shared.core.audio import ManagedAudioPlayer, music_enabled, sound_enabled
...
BATTLE_TEXT = {
    "de": {
        ...
        "shop_potion_item": "  • Heiltrank – {price} Gold (stellt 50% deiner Max-HP wieder her)",
        "shop_sigil_item": "  • Schutz-Siegel – {price} Gold (verleiht im naechsten echten Kampf ein Start-Schild)",
    ...
    "en": {
        ...
        "shop_potion_item": "  • Healing Potion – {price} gold (restores 50% of your max HP)",
        "shop_sigil_item": "  • Warding Sigil – {price} gold (grants a starting shield in the next real battle)",
...
def _localize_path_profile(profile: dict | None) -> dict:
    ...
    for key in ('title','rank','motif'):
        if profile.get(key):profile[key] = tr(profile[key],'en')
    return profile


def _localize_enemy_name(name: str) -> str:
    from shared.core.monster_catalog import enemy_display_name
    return enemy_display_name(name, _battle_ui_language())
...
def emit_gui_levelup(level: int, skill: str | None = None, max_hp: int | None = None):
    """The desktop renders a single notice instead of terminal emoji frames."""
    from shared.core import gui_bridge
    if not gui_bridge.enabled():
        return False
    gui_bridge.emit('level_up', level=level,
                    title=_localize_level_title(get_title_for_level(level)),
                    max_hp=max_hp if max_hp is not None else 100 + max(0, level - 1) * 10,
                    skill=skill, language=_battle_ui_language())
    return True


def run_levelup_fx(level: int, skill: str | None, plugin_dir: str, *, max_hp: int | None = None):
    """
    ...
    try:
        if emit_gui_levelup(level, skill, max_hp):
            return
        _lu_particles()
        _lu_banner(level)
        max_hp = max_hp if max_hp is not None else 100 + max(0, level - 1) * 10
...
class BattleMusicManager:
    def __init__(self, track_path: str | None, externally_managed=False):
        self.track_path = track_path
        # A dungeon owns one uninterrupted soundtrack across scenes and fights.
        self._player = None if externally_managed else ManagedAudioPlayer(track_path)

    def start(self):
        if self._player is None or not music_enabled():
            return
    ...
    def victory_jingle(self, path: str | None):
        if self._player is None or not sound_enabled() or not path or not os.path.isfile(path):
            return
...
class BattleState:
    ...
    def _load(self):
        if os.path.isfile(self.state_path):
            try:
                ...
                legacy_wins = int(s.get("fights_won", 0))
                s.setdefault("boss_progress", ((legacy_wins % 10 or 10) * 2) if legacy_wins else 0)
    ...
    def heal_after_chat(self):
        p = self.state["player"]
        p["hp"] = min(p["max_hp"], p["hp"] + max(1, p["max_hp"] // 5))
        self.state["flags"]["needs_heal"] = p["hp"] < p["max_hp"]
        self.save()
...
# 🎲 NAMENS-GENERIERUNG
# =====================================================
from shared.core.monster_catalog import ENEMY_PREFIXES, ENEMY_KINDS, ENEMY_SUFFIXES, BOSS_NAMES, FINAL_NAMES

def generate_enemy_name(state=None):
    from shared.core.monster_catalog import choose_enemy_name
    return choose_enemy_name(state, rng=random)
...
def print_level_bar(state_obj, last_gain: int | None = None):
    ...
    from shared.core import gui_bridge
    if gui_bridge.enabled():
        gui_bridge.emit('level_progress', level=lvl, xp=xp, next_xp=need, gain=last_gain)
        return
...
class BattleCore:
    ...
    def _profile_reactive_entrance(self, profile: dict, boss_profile: dict, ftype: str) -> str:
        ...
        if title.startswith(("Grenzhüter", "Boundary Keeper")):
            if en:
        ...
        if title.startswith(("Klangsucher", "Tone Seeker")):
            if en:
        ...
        if title.startswith(("Formträger", "Form Bearer")):
            if en:
    ...
    def _apply_enemy_weakness(self, attack_type: str, damage: int, turn_state: dict) -> tuple[int, str]:
        ...
        if weakness and attack_type == weakness:
            bonus = max(2, int(round(damage * (0.25 + turn_state.get('talents', {}).get('weakness', 0)/100))))
    ...
    def _gain_resonance(self, turn_state: dict, amount: int) -> int:
        ...
        amount += int(turn_state.get('talents', {}).get('resonance', 0))
    ...
    def _use_focus(self, player_hp: int, max_hp: int, turn_state: dict) -> tuple[int, str]:
        ...
        heal = class_talents.amplify(heal, turn_state.get('talents', {}), 'focus_heal')
        new_hp = min(max_hp, player_hp + heal)
        base_guard = 8 + int(max_hp * 0.05) + int(turn_state.get("focus_guard_bonus", 0))
        base_guard = class_talents.amplify(base_guard, turn_state.get('talents', {}), 'focus_guard')
    ...
    def _use_ult(self, level: int, max_hp: int, turn_state: dict) -> tuple[int, int, str]:
        ...
        damage = class_talents.amplify(damage, turn_state.get('talents', {}), 'impulse')
    ...
    def _localize_boss_profile(self, profile: dict) -> dict:
        ...
        localized["title"] = title_map.get(profile.get("title"), _localize_enemy_name(profile.get("title", "")))
    ...
    def _profile_boss_line(self, profile: dict, boss_profile: dict, outcome: str = "taunt") -> str:
        ...
        if outcome == "taunt":
            if title.startswith(("Grenzhüter", "Boundary Keeper")):
                if en:
                    return f"»So you come as {_localize_path_profile(profile).get('title',title)}. Then I will see whether your boundary still stands under pressure.«"
                return f"»Also kommst du als {title}. Dann werde ich sehen, ob deine Grenze auch unter Druck noch steht.«"
            if title.startswith(("Klangsucher", "Tone Seeker")):
                if en:
                    return f"»{_localize_path_profile(profile).get('title',title)}... good. Then I will break not only your body, but your rhythm.«"
    ...
    def _boss_special_attack(self, profile: dict, turn_state: dict, player_hp: int, max_hp: int) -> tuple[int, str]:
        ...
        if special == "Fraktur der Harmonie":
            ...
            dmg = class_talents.reduce_damage(dmg, turn_state)
    ...
    def _combat_achievement_catalog(self):
        if _battle_ui_language() == "en":
            return {
                "special_survived": "🛡 Survived Special",
                "phase2_reached": "🌗 Reached Phase 2",
                "boss_no_potion": "🚫 Boss Without Potion",
                "ult_finisher": "🌌 Resonance Finisher",
            }
        return {
    ...
    def _slow_line(
        ...
        """
        from shared.core.gui_bridge import enabled as gui_enabled
        if gui_enabled():
            print(text)
            return
...
# INNER BOSS
    ...
    def _auto_fight_type(self):
        from shared.core.terra_journey import next_fight_type
        return next_fight_type(self.state.state)

    # ---------- STATS / SCHADEN ----------
    def _base_attack_damage(self, attack_type: str, *, proportional=False) -> int:
        p = self.state.state["player"]
        lvl = max(1, p["level"])
        ...
        if proportional:
            return max(1, round((base + lvl * 2 + .5) * random.uniform(.95, 1.05)))
    ...
    def _reward_on_victory(self, ftype: str, xp_reward: int, context: dict | None):
        if (context or {}).get("editor_mode"):
            return ""
        s = self.state.state
        replay = bool((context or {}).get('terra_replay'))
        stats = s["stats"]
        world = s["world"]
        p = s["player"]
        ...
        stats["fights_won"] += 1
        source = (context or {}).get('combat_source')
        if replay:
            stats['terra_replay_wins'] = stats.get('terra_replay_wins', 0) + 1
        if ftype == 'final' and not replay:
            from shared.core.terra_journey import finish_final_retry
            finish_final_retry(s, context)
        if ftype == 'normal' and source in ('arena', 'random'):
            key = 'arena_wins' if source == 'arena' else 'random_wins'
            stats[key] = stats.get(key, 0) + 1
            if stats.get('final_wins', 0) < 5:
                stats['boss_progress'] = stats.get('boss_progress', 0) + (1 if source == 'arena' else 2)
        elif ftype == 'boss' and source in ('arena', 'random') and not replay:
            stats['boss_progress'] = max(0, stats.get('boss_progress', 0) - 20)

        if ftype == "boss" and not replay:
            stats["boss_fights"] += 1
            stats["boss_wins"] += 1
            ...
            if os.path.isfile(boss_scene):
                try:
                    if isinstance(context, dict) and callable(context.get("run_scene")):
                        context["run_scene"](boss_scene)
                    else:
                        subprocess.Popen(
                            ["python3", boss_scene],
        ...
        elif ftype == "final" and not replay:
            ...
            # Each narrative perspective gets its own one-time ending.
            from shared.core.story_campaign import companion_story_active
            credits_key = 'companion_credits_played' if companion_story_active() else 'credits_played'
            if world["principles_restored"] >= 5 and not world.get(credits_key, False):
                credits_py = os.path.join(self.plugin_dir, "credits.py")
                if os.path.isfile(credits_py):
                    try:
                        if isinstance(context, dict) and callable(context.get("run_scene")):
                            context["run_scene"](credits_py)
                        else:
                            subprocess.Popen(
                                ["python3", credits_py],
                                stdout=subprocess.DEVNULL,
        ...
        from shared.core.arena_difficulty import combat_tier
        difficulty = combat_tier(context)
        if (context or {}).get('combat_source') == 'arena':
            xp_effective //= 2
        if difficulty:
            xp_effective = max(0, int(xp_effective * difficulty['xp']))
        old_lvl, new_lvl = self.state.add_xp(xp_effective)

        # Level-Up Sound (einmalig pro Level-Up)
        if new_lvl > old_lvl:
            play_levelup_sound(self.plugin_dir)
            emit_gui_levelup(new_lvl, max_hp=p.get('max_hp'))
    ...
    def run_fight(self, ftype: str, context: dict | None = None) -> str:
        replay = (context or {}).get('terra_replay')
        if replay:
            from shared.core.terra_replay import resolve
            replay = resolve(self.state.state, replay.get('id'))
            if replay['kind'] != ftype:
                raise ValueError('Der gewählte Gegner passt nicht zum Kampftyp.')
        ...
        if ftype == "boss":
            boss_idx = replay['index'] if replay else s["stats"]["boss_wins"] + 1
            ...
            final_idx = replay['index'] if replay else s["stats"]["final_wins"] + 1
            ...
            enemy_name = boss_name_override or generate_enemy_name(None if title_demo_mode or guide_mode else s)

        enemy_name = _localize_enemy_name(enemy_name)

        enemy_hp, enemy_dmg_base, xp_reward = self._enemy_stats(ftype)
        from shared.core.arena_difficulty import combat_tier, normal_encounter_stats
        balanced_stats = normal_encounter_stats(
            enemy_hp, enemy_dmg_base, p['level'], ftype, context)
        if balanced_stats is not None:
            enemy_hp, enemy_dmg_base = balanced_stats
        ...
        difficulty = combat_tier(context)
        if difficulty and balanced_stats is None:
            enemy_hp = max(1, round(enemy_hp * difficulty['hp']))
            enemy_dmg_base = max(1, round(enemy_dmg_base * difficulty['damage']))
        ...
        boss_profile = self._localize_boss_profile(self._merge_boss_profile(boss_profile, battle_profile_cfg.get("boss_profile")))
        if not (context or {}).get("editor_mode"):
            self._register_boss_codex_entry(enemy_name, boss_profile, ftype)
        sigil_line = self._apply_starting_sigil(turn_state, context)
        talent_bonuses = class_talents.begin_fight(s, turn_state, context)
        attack_mult *= 1 + talent_bonuses.get('attack', 0)/100
        crit_chance = min(.75, crit_chance + talent_bonuses.get('crit', 0)/100)
        ...
        music = BattleMusicManager(battle_track if _exists(battle_track) else None,
                                   externally_managed=bool((context or {}).get('continuous_dungeon_music')))
        if isinstance(context, dict):
        ...
        if difficulty:
            source_label = {"arena":"Arena", "random":"Zufallskampf", "demo":"Demo"}.get((context or {}).get("combat_source"), "Kampf")
            from shared.core.gameplay_i18n import tr as gameplay_text
            language = _battle_ui_language()
            source_label = gameplay_text(source_label, language)
            tier_label = gameplay_text(difficulty['label'], language)
            tier_line = gameplay_text('{source} · {tier} · {xp} der normalen {source}-EP', language,
                                      source=source_label, tier=tier_label, xp=f"{difficulty['xp']:.0%}")
            if source_label == "Demo":
                tier_line = f"Demo · {tier_label}"
            self._slow_line(tier_line)
            log_lines.append(tier_line)
        ...
        try:
            ...
            while player_hp > 0 and enemy_hp > 0:
                # Potions and saves must see the current HP, not the value from
                # battle entry (which could restore HP without consuming a potion).
                p["hp"] = player_hp
                turn_counter += 1
                ...
                configured_weakness = enemy_cfg.get("weakness")
                turn_state["weakness"] = configured_weakness if configured_weakness in ("Harmonie", "Balance", "Schöpfungskraft", "Verbundenheit", "Respekt") else self._choose_enemy_weakness()
                aura_text = self._apply_enemy_aura(boss_profile, turn_state, turn_counter)
                from shared.core.gui_bridge import emit as gui_emit
                gui_emit("battle", active=True, enemy_name=enemy_name, round_number=turn_counter,
                         fight_type=ftype, enemy_level=enemy_cfg.get("level"), enemy_visual=battle_profile_cfg.get("visual", {}),
                         enemy_hp=max(0, enemy_hp), enemy_max_hp=max_enemy_hp,
                         player_hp=max(0 if guide_mode else 1, player_hp), player_max_hp=p["max_hp"], player_level=p.get("level", 1),
                         player_potions=int(p.get("potions", 0)),
                         resonance=turn_state.get("resonance", 0),
                         weakness=_principle_label(turn_state.get("weakness", "")),
                         aura=turn_state.get("enemy_aura") or "—",
                         phase="2" if turn_state.get("phase2_active") else "1",
                         charge=turn_state.get("enemy_charge", 0), charge_max=3,
                         combat_source=(context or {}).get("combat_source"),
                         arena_difficulty=(context or {}).get("arena_difficulty", "normal") if difficulty else None)
                print("\n------------------------------------------")
                ...
                from shared.core.gui_bridge import enabled as gui_enabled
                if not gui_enabled() or turn_counter % 3 == 0:
                ...
                if choice == "1":
                    ...
                    while True:
                        ...
                        base_dmg = self._base_attack_damage(atk_type, proportional=balanced_stats is not None)
                        ...
                        enemy_hp -= final_dmg
                        gui_emit("battle_effect", attacker="player", attack=atk_type if choice == "1" else ("skill" if choice == "2" else "impulse"), damage=int(final_dmg), enemy_name=enemy_name, enemy_hp=max(0, enemy_hp), player_hp=max(0 if guide_mode else 1, player_hp))
                        line = _battle_text(
                            "attack_use",
                    ...
                    if not p["skills"]:
                        ...
                        try:
                            ...
                            if idx == len(p["skills"]):
                                ...
                                final_dmg = class_talents.amplify(final_dmg, talent_bonuses, 'skill')
                    ...
                    gui_emit("battle_effect", attacker="player", attack="focus", damage=0,
                             heal=max(0, player_hp-hp_before_focus), player_hp=player_hp,
                             enemy_hp=max(0, enemy_hp), enemy_name=enemy_name)
                    if extra_txt:
                        line += extra_txt
                    self._slow_line(line)
                    ...
                    hp_before_potion = player_hp
                    potion_msg, did_heal = self._use_potion_in_fight(turn_state)
                    if did_heal:
                        turn_state["used_potion"] = True
                        gui_emit("battle_effect", attacker="player", attack="heal", damage=0,
                                 heal=max(0, p["hp"]-hp_before_potion), player_hp=p["hp"],
                                 player_potions=int(p.get("potions", 0)),
                                 enemy_hp=max(0, enemy_hp), enemy_name=enemy_name)
                    player_hp, extra_txt = self._modify_player_action(choice, turn_state, player_hp, p["max_hp"])
                    if extra_txt:
                    ...
                    if final_dmg > 0:
                        ...
                        p["hp"] = player_hp
                        gui_emit("battle_effect", attacker="player", attack="impulse", damage=int(final_dmg), enemy_name=enemy_name, enemy_hp=max(0, enemy_hp), player_hp=max(0 if guide_mode else 1, player_hp))
                        if enemy_hp <= 0:
                    ...
                    if random.random() < 0.5:
                        ...
                        if guide_mode:
                            ...
                            self.state.state["flags"]["needs_heal"] = True
                            if (context or {}).get("combat_source") not in ("arena","dungeon"):
                                self.state.state["stats"]["messages_since_last_fight"] = 0
                            self.state.save()
                        return "\n".join(log_lines)
                    else:
                ...
                if enemy_hp > 0:
                    if ftype in ("boss", "final"):
                        ...
                        if turn_state["enemy_charge"] >= 3:
                            ...
                            turn_state["enemy_charge"] = 0
                            gui_emit("battle_effect", attacker="enemy", attack="special", damage=max(0, hp_before_special-player_hp), enemy_name=enemy_name, enemy_hp=max(0, enemy_hp),
                                     player_hp_before=max(0 if guide_mode else 1, hp_before_special), player_hp=max(0 if guide_mode else 1, player_hp))
                    ...
                    if balanced_stats is not None:
                        # Keep damage variation proportional too; a flat +4 was
                        # much harsher at level 1 than at level 50.
                        dmg_in = max(1, round(enemy_dmg_base * random.uniform(.95, 1.05)))
                    else:
                    ...
                    dmg_in = class_talents.reduce_damage(dmg_in, turn_state)
                    ...
                    hp_before_hit = player_hp
                    player_hp -= dmg_in
                    gui_emit("battle_effect", attacker="enemy", attack="normal", damage=int(dmg_in), enemy_name=enemy_name, enemy_hp=max(0, enemy_hp),
                             player_hp_before=max(0 if guide_mode else 1, hp_before_hit), player_hp=max(0 if guide_mode else 1, player_hp))
        ...
        # Final HUD values are emitted before rewards or a cinematic begins.
        from shared.core.gui_bridge import emit as gui_emit
        gui_emit("battle", active=False, enemy_hp=max(0, enemy_hp),
                 player_hp=max(0 if guide_mode else 1, player_hp), enemy_name=enemy_name,
                 player_potions=int(p.get("potions", 0)),
        ...
        if player_hp > 0 and enemy_hp <= 0:
            ...
            if not title_demo_mode:
                ...
                if (context or {}).get('gui_mode') and reward_txt:
                    self._slow_line(reward_txt, delay_char=0.008, delay_line=0.3)
            combat_unlocks = [] if (context or {}).get("editor_mode") else self._evaluate_combat_achievements(ftype, turn_state)
            if combat_unlocks:
                ach_text = "\n".join(
                    (
                        f"🏆 New battle achievement: {title}"
                        if _battle_ui_language() == "en"
                        else f"🏆 Neuer Kampf-Erfolg: {title}"
                    )
                    for title in combat_unlocks
                )
            ...
            for extra_line in battle_profile_cfg.get("defeat_lines", []):
                self._slow_line(extra_line, delay_char=0.008, delay_line=0.25)
                log_lines.append(extra_line)
            if not ((context or {}).get("guide_mode") or (context or {}).get("editor_mode")):
                stats["fights_lost"] += 1
                # Only route progress retreats; completed wins remain in quest/history totals.
                from shared.core.terra_journey import GOAL, reset_after_defeat
                if reset_after_defeat(self.state.state, ftype, context):
                    arena_wins_needed = GOAL - stats['boss_progress']
                    random_wins_needed = (arena_wins_needed + 1) // 2
                    message = (f'🗺️ Back to the save point: win {random_wins_needed} random encounters or {arena_wins_needed} arena fights before trying again.'
                               if _battle_ui_language() == 'en' else
                               f'🗺️ Zurück zum Speicherpunkt: {random_wins_needed} Zufallssiege oder {arena_wins_needed} Arenasiege bis zum nächsten Versuch.')
                    self._slow_line(message, delay_char=0.008, delay_line=0.3)
...
class Plugin:
    ...
    commands = {
        "/fight": {
            "de": "Startet einen Arenakampf mit halben Kampf-EP (nach Freischaltung).",
            "en": "Starts an arena battle for half combat XP (after unlocking).",
        },
        "/fightboss": {
            "de": "Startet den Boss, wenn der Kampffortschritt ausreicht.",
            "en": "Starts the boss when sufficient combat progress has been earned.",
        },
        "/fightfinal": {
            "de": "Startet den Finalboss, wenn er an der Reihe ist.",
            "en": "Starts the final boss when it is due.",
        },
        "/fightmod": {
    ...
    def command(self, cmd, context=None):
        ...
        # ---------------- FIGHTS ----------------
        if base in {'/fight', '/fightboss', '/fightfinal'}:
            if not self.state.state['world'].get('combat_unlocked', False):
                return True, '🔒 Die Arena öffnet sich zusammen mit dem Zufallskampfmodus nach der Kampf-Einführung.'
            ftype = self.core._auto_fight_type()
            requested = {'/fightboss': 'boss', '/fightfinal': 'final'}.get(base)
            if requested and ftype != requested:
                return True, '🔒 Dieser Boss ist noch nicht an der Reihe. Spiele weiter in der Arena oder bestreite Zufallskämpfe.'
            arena_context = dict(context or {}, combat_source='arena')
            from shared.core.arena_difficulty import TIERS
            arena_context['arena_difficulty'] = random.choice(tuple(TIERS))
            arena_context.pop('guide_mode', None)
            arena_context.pop('scripted_actions', None)
            out = self.core.run_fight(ftype, arena_context)
            if isinstance(context, dict):
                context['reset_conversation_after_battle'] = True
            return True, '' if arena_context.get('gui_mode') else out
        ...
        if base == "/fightmod":
            ...
            if isinstance(context, dict):
                context["reset_conversation_after_battle"] = True
            return True, '' if mod_context.get('gui_mode') else out
        ...
        if base == "/xp":
            ...
            lines.append(f"• Arena-Siege: {stats.get('arena_wins', 0)} · Zufallssiege: {stats.get('random_wins', 0)}")
            lines.append(f"• Boss-Fortschritt: {stats.get('boss_progress', 0)}/20 (Arena +1 · Zufall +2)")
        ...
        if base == "/shop":
            ...
            if len(parts) >= 4 and parts[1].lower() == "buy":
                ...
                item = parts[2].lower()
                if item == "potion":
                    cost = item_price("potion", p.get("level", 1)) * amount
                elif item == "sigil":
                    cost = item_price("sigil", p.get("level", 1)) * amount
                else:
            ...
            lines.append(_battle_text("shop_potion_item", price=item_price("potion", p.get("level", 1))))
            lines.append(_battle_text("shop_sigil_item", price=item_price("sigil", p.get("level", 1))))
            lines.append("")
    ...
    def _build_hud(self, context=None) -> str:
        ...
        from shared.core import gui_bridge
        if gui_bridge.enabled():
            return ''  # The native character sidebar presents the live snapshot.
    ...
    def before_chat(self, user_input, context=None):
        ...
        # Commands are not chat messages and cannot trigger regeneration.
        if user_input.strip().startswith("/"):
            return False, user_input

        s = self.state.state
        stats, world = s["stats"], s["world"]
        story_state = self.core._load_story_state()
        played_story_ids = set(story_state.get("played", []))
        turn = context.get('gui_chat_turn')
        encounter = {'played': False}
        if turn and not turn.committed:
            turn.defer(lambda: self._credit_chat_message(user_input, context, not encounter['played']))
        else:
            self._credit_chat_message(user_input, context)
        ...
        if world["combat_unlocked"]:
            ms = stats["messages_since_last_fight"] + (1 if turn and not turn.committed else 0)
            trigger = False
            ...
            if trigger:
                ftype = self.core._auto_fight_type()
                from shared.core.arena_difficulty import TIERS
                fight_context = dict(context, combat_source='random', arena_difficulty=random.choice(tuple(TIERS)))
                from shared.core.terra_replay import take_pending, encounter_context
                replay = take_pending(self.state)
                if replay:
                    ftype, fight_context = encounter_context(self.state.state, replay['id'], context)
                encounter['played'] = True
                fight_text = self.core.run_fight(ftype, fight_context)
                self.state.save()

                # The GUI already presents the result in the arena. Only the
                # terminal replays the report after the following AI response.
                if isinstance(context, dict) and not context.get('gui_mode'):
    ...
    def _credit_chat_message(self, user_input, context, count_since_fight=True):
        """Commit chat rewards once; an earlier completed fight remains independent."""
        ...
        if new_lvl > old_lvl:
            run_levelup_fx(new_lvl, None, self.plugin_dir,
                           max_hp=self.state.state['player'].get('max_hp'))

        # -----------------------------------------------------
        # Nachrichten zählen
        # -----------------------------------------------------
        stats["messages_total"] += 1
        if count_since_fight:
    ...
    def _passive_regen_on_message(self):
        ...
        p["hp"] = new_hp
        s["flags"]["needs_heal"] = new_hp < max_hp
        self.state.save()
    ...
    def after_response(self, reply, context=None):
        ...
        if isinstance(context, dict) and context.get('gui_mode'):
            # Also discard any report queued by older/external hooks. The GUI
            # has its own HP/XP display and must not repeat combat in the chat.
            context.pop('battle_log', None)
            return reply
        ...
        # Healing happens only before the next player chat message.
        # The response to a message that triggered a defeat must leave HP at 1.
