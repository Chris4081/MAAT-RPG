...
class Plugin:
    ...
    def _relationship_state(self):
        story_state = self._load_state("companion_story_state.json" if companion_story_active() else "story_state.json")
    ...
    def before_chat(self, user_input: str, context=None):
        # In this perspective the player owns the AI voice; do not replace their
        # reply with a random AI monologue or short-circuit progression hooks.
        ...
        if random.randint(1, 40) == 1:
            ...
            # Flavour is display-only. A handled=True result would skip all
            # following plugins and the model in both GUI and terminal chat.
            # Keep the user's question intact and out of this lore message.
            print(f"\n{hint}\n")
