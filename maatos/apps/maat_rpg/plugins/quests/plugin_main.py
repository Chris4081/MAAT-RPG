...
class Plugin:
    ...
    def __init__(self, core=None, **kwargs):
        ...
        self.commands = {
            "/contracts": {"de":"Aufträge ab Level 50: /contracts buy <ID>","en":"Level 50 contracts: /contracts buy <ID>"},
            "/quests": {
    ...
    def _ui(self, text, **values):
        from shared.core.progression_i18n import tr
        return tr(text,self._lang(),**values)

    def _quest_display(self, quest: dict, language=None) -> dict:
        quest = dict(quest or {})
        from shared.core.story_campaign import companion_story_active
        if (language or self._lang()) != "en":
            if companion_story_active():
                from apps.maat_rpg.plugins.quests.companion_quests import display
                return display(quest)
            return quest
        from apps.maat_rpg.plugins.quests.english_content import display, keyword_aliases
        hints = keyword_aliases(quest) or QUEST_KEYWORD_ALIASES.get(quest.get('id'), [])
        # Existing universal markers (Mona Lisa, PLP, formulas) also remain valid.
        if not hints:
            hints = [k for k in quest.get('keywords', []) if k.lower() in ('mona lisa','plp','b_universe','maat_master') or k.lower().startswith(('maat value','calculate','my ','explain '))]
        return display(quest, QUEST_I18N.get(quest.get('id'), quest), companion_story_active(), hints)
    ...
    def _matches_chat_keyword_quest(self, q: dict, user_input: str) -> bool:
        ...
        qid = q.get("id", "")
        from apps.maat_rpg.plugins.quests.english_content import keyword_aliases
        keywords = [self._normalize_keyword_text(k) for k in (q.get("keywords", []) + QUEST_KEYWORD_ALIASES.get(qid, []) + keyword_aliases(q))]
    ...
    def _find_quest_in_list(self, quests: list, qid: str):
        ...
        for quest in quests:
            ...
            if needle == quest_id or needle == quest_name or needle == self._quest_name(quest).strip().lower():
                return quest
        return None

    def _story_state_path(self) -> str:
        return story_state_file()
    ...
    def buy_contract(self, quest_id):
        if self.state is None:
            return self._ui('Spielstand ist noch nicht bereit.')
        player = self.state.state['player']
        if int(player.get('level', 1)) < 50:
            return self._ui('Das Auftragsbrett ist ab Level 50 verfügbar.')
        quest = next((q for q in self.qstate['locked'] if q.get('id') == quest_id and q.get('purchase_price')), None)
        if quest is None:
            return self._ui('Dieser Auftrag wurde bereits gekauft oder ist unbekannt.')
        cost = quest['purchase_price']
        if player.get('gold', 0) < cost:
            return self._ui('Nicht genug Gold: Dieser Auftrag kostet {cost} Gold.',cost=cost)
        from copy import deepcopy
        active = deepcopy(quest)
        source = self.qstate['meta'].get('messages_total', 0) if quest.get('counter_key') == 'messages_total' else self.state.state.get('stats', {}).get('fights_won', 0)
        active.update(progress=0, counter_start=int(source), purchased_at=datetime.now().isoformat())
        player['gold'] -= cost
        self.qstate['locked'].remove(quest)
        self.qstate['active'].append(active)
        try:
            self.state.save()
        except Exception:
            player['gold'] += cost
            self.qstate['active'].remove(active)
            self.qstate['locked'].append(quest)
            raise
        return self._ui('📜 Auftrag gekauft: {name} · −{cost} Gold. Das Ziel zählt ab jetzt; du findest es im Questlog.',name=self._quest_name(quest),cost=cost)

    def contract_board(self):
        entries = []
        for group in ('locked', 'active', 'completed'):
            for q in self.qstate.get(group, []):
                if q.get('purchase_price'):
                    translations = {lang: {key: value for key, value in self._quest_display(q, lang).items()
                                           if key in ('name', 'desc')} for lang in ('de', 'en')}
                    entries.append(dict(self._quest_display(q), status=group, effective_xp=self._quest_xp_effective(q),
                                        path_bonus=self._quest_path_bonus_preview(q), translations=translations))
        return sorted(entries, key=lambda q:q['id'])
            ...
            for lst in (qs["available"], qs["active"], qs["completed"], qs["locked"]):
                for q in lst:
                    ...
                    if definition:
                        ...
                        changed = True
            # Add newly shipped definitions without resetting existing progress.
            known_ids = {q.get('id') for group in ('available','active','completed','locked') for q in qs[group]}
            for definition in _ALL_QUEST_DEFINITIONS:
                if definition['id'] not in known_ids:
                    from copy import deepcopy
                    qs['locked'].append(deepcopy(definition))
    ...
    def command(self, full_cmd: str, context=None):
        ...
        if base == "/contracts":
            if len(args)==2 and args[0]=="buy":
                return True, self.buy_contract(args[1])
            return True, "\n\n".join(f"{q['id']} · {q['name']} · {q['purchase_price']} Gold\n{q['desc']}" for q in PAID_CONTRACTS)
    ...
    def _cmd_list(self, args):
        ...
        paid_count = sum(bool(q.get("purchase_price")) for q in self.qstate.get("locked", []))
        if paid_count:
            lines.append(f"\n📜 {paid_count} kaufbare Aufträge ab Level 50: /contracts")
        locked_count = len(self.qstate.get("locked", [])) - paid_count
        if locked_count:
            lines.append(self._t(
    ...
    def _cmd_quest(self, args):
        ...
        if sub == "accept":
            ...
            if not q:
                ...
                if locked_q:
                    if locked_q.get("purchase_price"):
                        return f"📜 Diesen Auftrag kaufst du ab Level 50 am Auftragsbrett: /contracts buy {locked_q['id']}"
                    return self._t(
            ...
            if q.get("purchase_price"):
                return "Kaufbare Aufträge werden über /contracts gekauft."
    ...
    def _quest_progress_text(self, q: dict | None) -> str:
        ...
        if qtype == "daily_streak":
            ...
            return self._t(f"Fortschritt: {cur}/{target} Tage", f"Progress: {cur}/{target} {'day' if target == 1 else 'days'}")
        ...
        if qtype in ("counter", "battle_win"):
            ...
            if q.get("counter_key") == "messages_total":
                ...
                if q.get("purchase_price"):
                    meta_total = max(0, meta_total - int(q.get("counter_start", meta_total)))
    ...
    def _quest_unlock_level(self, quest: dict) -> int:
        return int(quest.get('minimum_level', max(1, int(quest.get('level_tier', 0)) * 2)))

    def _level_quest_eligible(self, quest: dict) -> bool:
        if quest.get('purchase_price') or int(quest.get('level_tier', 0)) <= 0:
            return False
        if self._get_current_level() < self._quest_unlock_level(quest):
            return False
        root = self.state.state if self.state is not None else {}
        return not quest.get('requires_combat') or bool(root.get('world', {}).get('combat_unlocked'))
    ...
    def _check_unlocks(self):
        ...
        if intro_shown or meta.get("quest_intro_shown", False):
            ...
            # A returning high-level player may receive new quests below their
            # already visited tier; those must unlock without another level-up.
            eligible = any(self._level_quest_eligible(q) for q in self.qstate['locked'])
            if target_tier > unlocked_tier or eligible:
                meta["last_unlock_level"] = current_level
                tier_newly_active = []

                for q in list(self.qstate["locked"]):
                    q_tier = int(q.get("level_tier", 0))
                    if self._level_quest_eligible(q):
                        self.qstate["locked"].remove(q)
                        active_q = dict(q)
                        if active_q.get("type") in ("counter", "daily_streak", "battle_win"):
                            active_q.setdefault("progress", 0)
    ...
    def _matches_daily_quest(self, quest: dict, user_input: str) -> bool:
        normalized = self._normalize_keyword_text(user_input)
        from apps.maat_rpg.plugins.quests.english_content import keyword_aliases
        keywords = [self._normalize_keyword_text(k) for k in self._daily_quest_keywords(quest) + keyword_aliases(quest)]
    ...
    def _refresh_daily_quests(self):
        """Reopen only explicit daily-repeat quests after a calendar-day change."""
        today = datetime.now().date().isoformat()
        changed = False
        for quest in list(self.qstate['completed']):
            if not quest.get('daily_reset'):
                continue
            last = str(quest.get('last_reward_day') or str(quest.get('completed_at',''))[:10])
            if not last or last >= today:
                continue
            self.qstate['completed'].remove(quest)
            quest.pop('completed_at', None)
            quest.pop('last_progress_day', None)
            quest['progress'] = 0
            self.qstate['active'].append(quest)
            changed = True
        if changed:
            self._save_runtime_state()
    ...
    def refresh_paid_contracts(self):
        before = [(q["id"], q.get("progress", 0)) for q in self.qstate["active"] if q.get("purchase_price")]
        messages = []
        self._check_battle_quests(messages, paid_only=True)
        after = [(q["id"], q.get("progress", 0)) for q in self.qstate["active"] if q.get("purchase_price")]
        if before != after:
            self._save_runtime_state()
        return messages

    def refresh_combat_quests(self):
        """Synchronize real counters after gameplay, without a synthetic chat turn."""
        before = [(group, q['id'], q.get('progress', 0))
                  for group in ('locked', 'active', 'completed') for q in self.qstate[group]]
        messages = self._check_unlocks()
        self._check_battle_quests(messages)
        after = [(group, q['id'], q.get('progress', 0))
                 for group in ('locked', 'active', 'completed') for q in self.qstate[group]]
        if before != after:
            self._save_runtime_state()
        return messages

    def _check_battle_quests(self, completed_msgs: list, paid_only=False):
        """
        ...
        try:
            ...
            counters = progress_counters(root)
        except Exception:
            return

        for q in list(self.qstate["active"]):
            if paid_only and not q.get("purchase_price"):
                continue
            qtype = q.get("type")
            ...
            if q.get("purchase_price"):
                progress_source = max(0, progress_source - int(q.get("counter_start", progress_source)))
    ...
    def _complete_quest(self, quest: dict, completed_msgs: list):
        ...
        """
        if quest not in self.qstate['active']:
            return
        from shared.core.talents import grants_quest_point
        talent_point = grants_quest_point(quest) and not any(
            q.get('id') == quest.get('id') for q in self.qstate['completed'] if isinstance(q, dict))
        if quest.get('daily_reset'):
            today = datetime.now().date().isoformat()
            if str(quest.get('last_reward_day', '')) >= today:
                return
            quest['last_reward_day'] = today
            quest['completions'] = int(quest.get('completions', 0)) + 1
        self.qstate['active'].remove(quest)

        quest["completed_at"] = datetime.now().isoformat()
        self.qstate["completed"].append(quest)

        if quest.get('purchase_price') and self.state is not None:
            player = self.state.state['player']
            for key, amount in quest.get('reward_items', {}).items():
                player[key] = int(player.get(key, 0)) + amount
            title = quest.get('reward_title')
            titles = player.setdefault('earned_titles', [])
            if title and title not in titles:
                titles.append(title)
            from apps.maat_rpg.plugins.quests.english_content import TITLES
            display_title = TITLES.get(title,title) if self._lang() == 'en' else title
            completed_msgs.append('🎁 ' + ', '.join(f'{amount} ' + self._ui('Heiltränke' if key == 'potions' else 'Schutz-Siegel') for key,amount in quest.get('reward_items', {}).items()) + (self._ui(' · Titel: {title}',title=display_title) if title else ''))
        ...
        if talent_point:
            completed_msgs.append(self._t('✦ +1 Talentpunkt aus einer Kampfquest · nach der Klassenwahl nutzbar.',
                                          '✦ +1 combat quest talent point · available after choosing a class.'))
