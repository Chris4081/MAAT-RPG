...
class Plugin:
    ...
    def __init__(self):
        ...
        self.state_path = story_state_file()
    ...
    def _start_music_once(self, filename: str, entry: dict | None = None):
        ...
        if path is None or not path.is_file():
            # Source editions intentionally omit the optional soundtrack.
            return
    ...
    def _load_story_module(self, module_name: str, entry: dict | None = None):
        ...
        if companion_story_active():
            from gui.companion_stories import CompanionStory, SCENES
            if module_name in SCENES:
                return CompanionStory(module_name, self._language())
    ...
    def _run_story_interactive(self, story, music_filename: str = None, extra_lines: list[str] | None = None, entry: dict | None = None):
        ...
        """
        presenter = getattr(self, 'gui_present_story', None)
        if callable(presenter):
            ...
            music = resolve_story_asset_path(music_filename, self.story_dir, entry) if music_filename else None
            presenter(lines, str(music) if music else None, entry or {})
            if isinstance(choice, dict):
                self._handle_story_choice(choice)
            return
    ...
    def _handle_story_choice(self, choice_result: dict):
        ...
        while True:
            hint = ("Choose a decision — it shapes your journey!" if self._language() == "en"
                    else "Wähle eine Entscheidung – sie prägt deinen Spielverlauf!")
            raw = input(hint + "\n" + self._t("choice_prompt", count=len(options))).strip()
            try:
                index = int(raw) - 1
            except ValueError:
    ...
    def _build_path_profile(self) -> dict | None:
        ...
        return {
            "title": self._path_text(title),
            "rank": self._path_text(rank),
            "motif": self._path_text(motif),
        }

    def _path_text(self,text):
    ...
    # -------------------------------------------------
    def _find_story_for_message_count(self, pending=0):
        """
        ...
        """
        msg_count = self.state.get("messages_total", 0) + pending
        ...
        for entry in self.config.get("stories", []):
            ...
            from shared.core.story_campaign import companion_story_active
            threshold = int(trig.get("messages", -1))
            if msg_count == threshold or (companion_story_active() and 0 <= threshold <= msg_count):
                return entry
    ...
    def _find_story_for_state(self):
        ...
        for entry in self.config.get("stories", []):
            ...
            from shared.core.story_campaign import companion_story_active
            target = trig.get("equals")
            if value == target or (companion_story_active() and isinstance(value, (int, float)) and isinstance(target, (int, float)) and value >= target):
                return entry
    ...
    def before_chat(self, user_input: str, context=None):
        ...
        turn = (context or {}).get('gui_chat_turn')
        pending = int(bool(turn and not turn.committed))
        def credit_message():
            self.state['messages_total'] += 1
            self._save_state()
        if pending:
            turn.defer(credit_message)
        else:
            credit_message()

        from shared.core.story_campaign import companion_story_active
        if companion_story_active():
            entry = self._find_story_for_message_count(pending) or self._find_story_for_state()
            if entry:
        ...
        entry = self._find_story_for_message_count(pending)
        if not entry:
            return False, user_input
    ...
    def _play_reflection(self, reflection: dict):
        ...
        from shared.core.story_campaign import companion_story_active
        if companion_story_active():
            from gui.companion_stories import reflection_text
            lines = [reflection_text(key, self._language())]
        else:
        ...
        presenter = getattr(self, 'gui_present_story', None)
        if callable(presenter):
            track = resolve_story_asset_path(music, self.story_dir, None) if music else None
            companion_title = 'The voice in the light · Memory' if self._language() == 'en' else 'Die Stimme im Licht · Erinnerung'
            presenter(lines, str(track) if track else None, {'module':'reflection','name':companion_title if companion_story_active() else self._t('reflection_title')})
        else:
    ...
    def _play_story_entry(self, entry, context=None):
        from shared.core.story_i18n import story_entry
        entry = story_entry(entry, self._language())
        from shared.core.story_campaign import companion_story_active
        if companion_story_active():
            from gui.companion_stories import SCENES, scene_payload
            entry.pop('profile', None)
            if entry.get('module') in SCENES:
                entry['name'] = scene_payload(entry['module'], self._language())['name']
        story_obj = self._load_story_module(entry.get("module"), entry)
        if story_obj:
            music_file = entry.get("music")
            scene_lines = [] if companion_story_active() else self._profile_scene_lines(entry.get("module", ""))
