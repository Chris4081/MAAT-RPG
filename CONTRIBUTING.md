...
For the desktop GUI, follow [GUI-START.md](GUI-START.md). The Linux installer
installs the GUI and native CPU backend. macOS uses an isolated Python environment
and the architecture-specific GGUF build described there. Windows is experimental.
Do not run the older terminal setup when you only want the GUI environment.
...
See the [developer guide](docs/REPOSITORY.md#english). No model is needed for
these checks; the GUI test dependencies are required for the second command.
...
- `README.md`
  - user-facing overview in English (default)
- `README.de.md`
  - German overview
- `FUNKTIONEN.md`
  - feature overview
